...
public class NewBehaviourScript : MonoBehaviour
{
    ...
    void Update()
    {
        Quaternion rotationRight = AdjustRotationAxis(rightWheel.transform.rotation);
        Quaternion rotationLeft = AdjustRotationAxis(leftWheel.transform.rotation);
        ...
        if (timeSinceLastMessage >= messageInterval)
        {
            ...
            jsonMessage += GetRotationDataAsJson(rightWheel, "RightWheel", rotationRight) + ",";
            jsonMessage += GetRotationDataAsJson(leftWheel, "LeftWheel", rotationLeft);
            jsonMessage += "}";
    ...
    private void ParseAndUpdateGameObject(JObject jsonMessage, string key, GameObject gameObject)
    {
        ...
        {
            var positionData = jsonMessage[key]["position"];
            var rotationData = jsonMessage[key]["rotation"];
            ...
            if (positionData != null && rotationData != null)
            {
                Vector3 position = new Vector3(
                    ...
                    (float)positionData["z"]
                );
                Quaternion rotation = new Quaternion(
                    (float)rotationData["x"],
                    (float)rotationData["y"],
                    (float)rotationData["z"],
                    (float)rotationData["w"]
                );
